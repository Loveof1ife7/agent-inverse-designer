...
其中保存的是原始 `node_data` 和 `element_conn`。预处理后的训练字段 `coordinates` 指归一化到 `[0,1]^3` 的节点坐标，不再使用 `geometry` 这个目录名，避免和 InverseDesigner 中的 geometry 概念混淆。
...
其中 `Strain` 为工程应变，无量纲；`Stress` 为工程应力，单位 MPa。训练数据中的 `y` 不直接保存变长原始曲线，而是统一转换为固定长度采样向量：
...
```text
y[0]  = stress at strain 0.00
y[1]  = stress at strain 0.01
...
y[30] = stress at strain 0.30
```
...
6. 如果曲线没有覆盖到 0.30，右侧使用 `hold_last`，并在 metadata 中记录 `extrapolated_point_count`。
...
下面是缩略示例。实际文件中 `y` 和 `property.strain_grid` 都是 31 维。
...
当前预处理数据使用 `bfs_parent_pointer_reindexed_v2`：

```text
1. 从原始 node 1 开始做 BFS，邻居按原始编号升序访问。
2. 按 BFS discovery order 重新编号节点。
3. `coordinates`、`edges`、`extra_edges` 都使用重编号后的 1-based index。
4. `parent_sequence = [p_2, ..., p_n]`，严格满足 `p_j < j`，适合下游自回归训练。
5. 原始节点顺序保存在 `preprocessing.node_order_original_ids`。
```
